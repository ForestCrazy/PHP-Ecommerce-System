...
function addFavoriteSelectItemFromCart() {
  ...
  if (productSelect.length > 0) {
    Swal.fire({
      title:
        "เมื่อเพิ่มสินค้าในรายการสินค้าที่ชอบแล้ว ต้องการนำสินค้าที่เลือกออกจากตะกร้าสินค้าด้วยหรือไม่?",
      text: "",
      icon: "warning",
      showDenyButton: true,
      showCancelButton: true,
...
function createAddress() {
  ...
  $.ajax({
    ...
    success: function (res) {
      var resp = JSON.parse(res);
      if (resp.success) {
        Swal.fire("เพิ่มที่อยู่สำเร็จ", "", "success").then(() => {
          window.location.href = window.location.href;
        });
      } else {
        Swal.fire("เกิดข้อผิดพลาด", "เพิ่มที่อยู่ไม่สำเร็จ", "error");
      }
    },
  });
}
